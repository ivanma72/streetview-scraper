...
@gen.coroutine
def _panoids_data_async(lat, lon):
    """
    Gets the response of the script on Google's servers that returns the
    closest panoramas (ids) to a give GPS coordinate.
    """
    ...
    raise gen.Return(response.body.decode('utf-8'))
    # return requests.get(url)

@gen.coroutine
def panoids_async(lat, lon, closest=False, disp=False):
    """
    ...
    resp = yield _panoids_data_async(lat, lon)
    ...
    if closest:
        raise gen.Return([pans[i] for i in range(len(dates))])
    else:
        raise gen.Return(pans)
